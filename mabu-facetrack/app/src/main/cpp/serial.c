...
Java_com_mabu_facetrackadb_SerialPort_openTty(JNIEnv* env, jclass cls,
    ...
    tio.c_cflag = (tio.c_cflag & ~(CSIZE | PARENB | CSTOPB | CRTSCTS | HUPCL)) |
                  CS8 | CLOCAL | CREAD;
    tio.c_iflag = 0;
    tio.c_oflag = 0;
    tio.c_lflag = 0;
...
JNIEXPORT jint JNICALL
Java_com_mabu_facetrackadb_SerialPort_writeBytes(JNIEnv* env, jclass cls,
...
Java_com_mabu_facetrackadb_SerialPort_closeTty(JNIEnv* env, jclass cls, jint fd) {
    if (fd >= 0) close(fd);
}
